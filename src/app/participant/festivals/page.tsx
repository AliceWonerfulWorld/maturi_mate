...
export default function ParticipantFestivalsPage() {
  ...
  // 祭りの平均評価を計算
  const getFestivalRating = (festivalId: string) => {
    ...
    <Link href={`/participant/festivals/${festival.id}`} prefetch={false}>
      <Card className="group hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 bg-white/90 backdrop-blur-sm border-0 shadow-xl overflow-hidden will-change-transform gpu-accelerated contain-layout">
        ...
        <CardHeader className="pb-3 relative">
          <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-br from-blue-500/10 to-purple-500/10 rounded-full transform translate-x-8 -translate-y-8"></div>
          <div className="relative z-10">
            <div className="flex justify-between items-start mb-2">
              <CardTitle className="text-lg font-bold text-gray-800 group-hover:text-blue-600 transition-colors pr-2">
                {festival.name}
              </CardTitle>
